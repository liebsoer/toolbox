...
const { workflow, storage, Item, utils } = AlfredNode;
...
describe('#Integration test', () => {
  ...
  const search = function (query) {
    _.each(
      utils.filter(query, _getData(), (data) => data.name),
      (item) => {
        workflow.addItem(
          new Item({
            title: item.name,
  ...
  it('interation test', () => {
    (function main() {
      ...
      AlfredNode.run();
      assert.strictEqual(
        `{"items":[{"valid":"NO","autocomplete":"Kat${utils.SUB_ACTION_SEPARATOR}","title":"Kat","subtitle":"10"}]}`,
        feedback
      );
      workflow.clearItems();
      ...
      AlfredNode.run();
      assert.strictEqual(
        `{"items":[{"valid":"NO","autocomplete":"Alex${utils.SUB_ACTION_SEPARATOR}","title":"Alex","subtitle":"20"},{"valid":"NO","autocomplete":"David${utils.SUB_ACTION_SEPARATOR}","title":"David","subtitle":"30"},{"valid":"NO","autocomplete":"Kat${utils.SUB_ACTION_SEPARATOR}","title":"Kat","subtitle":"10"}]}`,
        feedback
      );
      workflow.clearItems();
      ...
      process.argv = [
        ...
        '',
        'action',
        `Alex${AlfredNode.utils.SUB_ACTION_SEPARATOR}abc`,
      ];
      AlfredNode.run();
      assert.strictEqual(
        '{"items":[{"valid":"NO","title":"Menu Item 1: abc","subtitle":"Alex"}]}',
        feedback
      );
      workflow.clearItems();
      ...
      process.argv = [
        ...
        '',
        'action',
        `Alex${AlfredNode.utils.SUB_ACTION_SEPARATOR}`,
      ];
      AlfredNode.run();
      ...
      AlfredNode.run();
      assert.strictEqual(
        '{"items":[{"valid":"NO","autocomplete":"David $>","title":"David","subtitle":"30"},{"valid":"NO","autocomplete":"Kat $>","title":"Kat","subtitle":"10"},{"valid":"NO","autocomplete":"Alex $>","title":"Alex","subtitle":"20"}]}',
        feedback
      );
      workflow.clearItems();
      storage.clear();
